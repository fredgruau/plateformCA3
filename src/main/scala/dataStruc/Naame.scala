...
/** suggestion alternative de chatgpt pour getAllFieldValues, quand la reflection faisait planter le debug. */
    def getAllFieldValues34(x: Any): Seq[(String, Any)] = {
      ...
      try { val m   = runtimeMirror(cl) // appel à runtimeMirror/reflect
      } catch {
        case e: Throwable =>
          e.printStackTrace()
          println("TCCL   = " + Thread.currentThread.getContextClassLoader)
          println("instCL = " + x.getClass.getClassLoader)
      }
      ...
      val im  = m.reflect(x)
      val tpe = m.classSymbol(x.getClass).toType // <- pas instanceMirror.symbol.toType

      val getters =
        tpe.members.collect{ case m: MethodSymbol if m.isGetter && m.isPublic && m.paramLists.isEmpty => m }.toList

      getters.reverse.map(g => g.name.toString.trim -> im.reflectMethod(g).apply())
    }
