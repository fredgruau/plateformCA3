...
import compiler.ASTBfun.{addRedop, andRedop, derivative, maxSign, maxSignRedop, minSignRedop, orRedop, p, redop, uI2SI, xorRedop}
...
object Grad {
  ...
  val slopeDeltaRadiusDef: Fundef2[(V, SI),(T[V, E], B), ((T[V, E], B), ((V, SI), ((T[V,E], SI), (E, B))))] =  {
    ...
    val r = pL[V, SI]("dis")
    /** indicates neighbors nearer to the source of propagation */
    val srcPropag = pL[T[V, E], B]("srcPropag")
    /** opposite of radius */
    val ropp = -r
    /** we have distance on one side, and opposite of distance on the other side of boolE */
    val se: IntVe = send(List(r, r, r, ropp, ropp, ropp))
  ...
  def deltaCall(d: IntV, opDef:siFieldOperator ): (BoolVe, IntV, BoolE, BoolE) = {
    val r = Call1(opDef, d);
    val e1 = new Heead(r) with BoolVe
    val t = new Taail(r)
    val e2 = new Heead(t) with IntV
