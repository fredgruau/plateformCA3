...
object Grad {
  ...
  /** macro that does all the computation needing to compute and broadcast the radius From an IntV
   * , computes the gradient sign, slopgt
   ...
   * where the gradient is 4 in absolute value, which makes it impossible to decide the slope sign, if three bits are used.
   * If four bits are used, it should be 8,
   * however, 4 is still useful, because if there is no gap, then this is a strong hint that 3 bits could be used
   * The vortex can be computed in a second step.
   * */
  val slopeDeltaRadiusDef: Fundef1[(V, SI), ((T[V, E], B), ((V, SI), ((E, B), (E, B))))] =  {
    val d = pL[V, SI]("dis")
    ...
    val gap: BoolE = eq0(grad3 + 4)   /** builds the corresponding boolEv, by negating on one side but not the other  */
    ...
    /** what needs to be added to the distance to the source, in order to adjust the movement of the source */
  ...
  /** Calls slopeDeltaRadiusDef, and separate the four results. */
  def slopDeltaRadius(d: IntV): (BoolVe, IntV, BoolE, BoolE) = {
    val r = Call1(slopeDeltaDistDef, d);
...
type siFieldOperator=Fundef1[(V, SI), ((T[V, E], B), ((V, SI), ((E, B), (E, B))))]
  ...
  val slopeDeltaDistDef: Fundef1[(V, SI), ((T[V, E], B), ((V, SI), ((E, B), (E, B))))] =  {
    ...
    val grad3: IntE = reduce(addRedop[SI].asInstanceOf[redop[SI]], transfer(se))
    /**  true iff the two neighbors cannot be compared. It should be 8 instead of 4,  if the number of bits is 4 */
    val gap: BoolE = eq0(grad3 + 4) //& chip.borderE.df   /** builds the corresponding boolEv, by negating on one side but not the other  */
    /** when sending back from boolE to boolEV, we have to take the opposite again,  towards the center */
    val grad6: IntEv = send(List(-grad3, grad3))
    /** we need to compare to 0 */
    val slopEv: BoolEv = ltSI(grad6)
  ...
  def minmaxCall(d: IntV,opDef:siFieldOperator ): (BoolVe, IntV, BoolE, BoolE) = {
    val r = Call1(opDef, d);
    val e1 = new Heead(r) with BoolVe
    val t = new Taail(r)
    val e2 = new Heead(t) with IntV
