...
// initialisation 
 int auxL110=0,shiftshrink=0,shrink=0;
for (int i = 1; i < pshrink$es.length -1; i++) {
 auxL110= pshrink$en[i] ;shrink= pshrink$es[i] ;redTshrinkFE$e[i]=( shrink  &  auxL110 );shiftshrink= shrink ;
 ...
 redTshrinkFE$ne[i]=( auxL110  &  shiftshrink );
  }
  ;
...
// initialisation 
 int auxL82=0,shiftshrink=0,shrink=0;
for (int i = 1; i < pshrink$e.length -1; i++) {
 auxL82= pshrink$e[i] ;shrink= pshrink$se[i] ;redTshrinkEF$es[i]=( shrink  &  auxL82 );shiftshrink= shrink ;
 ...
 redTshrinkEF$en[i]=( auxL82  &  shiftshrink );
  }
  ;
