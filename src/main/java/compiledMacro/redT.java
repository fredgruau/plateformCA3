...
// initialisation 
 int auxL121=0,shiftshrink=0,shrink=0;
for (int i = 1; i < pshrink$es.length -1; i++) {
 auxL121= pshrink$en[i] ;shrink= pshrink$es[i] ;redTshrinkFE$e[i]=( shrink  &  auxL121 );shiftshrink= shrink ;
 ...
 redTshrinkFE$ne[i]=( auxL121  &  shiftshrink );
  }
  ;
...
// initialisation 
 int auxL97=0,shiftshrink=0,shrink=0;
for (int i = 1; i < pshrink$e.length -1; i++) {
 auxL97= pshrink$e[i] ;shrink= pshrink$se[i] ;redTshrinkEF$es[i]=( shrink  &  auxL97 );shiftshrink= shrink ;
 ...
 redTshrinkEF$en[i]=( auxL97  &  shiftshrink );
  }
  ;
