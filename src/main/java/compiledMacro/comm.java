...
// initialisation 
 int tmun06$0=0,tmun06$1=0,tmun06$2=0,tmun06$3=0,tmun07$0=0,tmun07$1=0,tmun07$2=0,tmun07$3=0,tmun08$0=0,tmun08$1=0,tmun08$2=0,tmun08$3=0;
for (int i = 1; i < pngh$e$0.length -1; i++) {
 ...
 ver$w$0[i-1]=( tmun06$0  >>>  1 );ver$w$1[i-1]=( tmun06$1  >>>  1 );ver$w$2[i-1]=( tmun06$2  >>>  1 );ver$w$3[i-1]=( tmun06$3  >>>  1 );
 tmun06$0= pngh$e$0[i] ;tmun06$1= pngh$e$1[i] ;tmun06$2= pngh$e$2[i] ;tmun06$3= pngh$e$3[i] ;
 ver$nw$0[i]= tmun07$0 ;ver$nw$1[i]= tmun07$1 ;ver$nw$2[i]= tmun07$2 ;ver$nw$3[i]= tmun07$3 ;
 tmun07$0= pngh$se$0[i] ;tmun07$1= pngh$se$1[i] ;tmun07$2= pngh$se$2[i] ;tmun07$3= pngh$se$3[i] ;
 ver$ne$0[i]=( tmun08$0  <<  1 );ver$ne$1[i]=( tmun08$1  <<  1 );ver$ne$2[i]=( tmun08$2  <<  1 );ver$ne$3[i]=( tmun08$3  <<  1 );
 tmun08$0= pngh$sw$0[i] ;tmun08$1= pngh$sw$1[i] ;tmun08$2= pngh$sw$2[i] ;tmun08$3= pngh$sw$3[i] ;
  }
  ;
p.prepareBit(ver)
 ;
...
// initialisation 
 int tmun29$0=0,tmun29$1=0,tmun29$2=0,tmun29$3=0,tmun30$0=0,tmun30$1=0,tmun30$2=0,tmun30$3=0;
for (int i = 1; i < pdistantEdgeef$es$0.length -1; i++) {
 apexEf$h1$0[i]=( tmun29$0  <<  1 );apexEf$h1$1[i]=( tmun29$1  <<  1 );apexEf$h1$2[i]=( tmun29$2  <<  1 );apexEf$h1$3[i]=( tmun29$3  <<  1 );
 tmun29$0= pdistantEdgeef$s$0[i] ;tmun29$1= pdistantEdgeef$s$1[i] ;tmun29$2= pdistantEdgeef$s$2[i] ;tmun29$3= pdistantEdgeef$s$3[i] ;
 ...
 apexEf$ad2$0[i-1]=( tmun30$0  >>>  1 );apexEf$ad2$1[i-1]=( tmun30$1  >>>  1 );apexEf$ad2$2[i-1]=( tmun30$2  >>>  1 );apexEf$ad2$3[i-1]=( tmun30$3  >>>  1 );
 tmun30$0= pdistantEdgeef$es$0[i] ;tmun30$1= pdistantEdgeef$es$1[i] ;tmun30$2= pdistantEdgeef$es$2[i] ;tmun30$3= pdistantEdgeef$es$3[i] ;
  }
  ;
p.prepareBit(apexEf)
 ;
...
// initialisation 
 int tmun13=0,tmun14=0;
for (int i = 1; i < pdistantEdgeef$es.length -1; i++) {
 apexEf$h1[i]=( tmun13  <<  1 );tmun13= pdistantEdgeef$s[i] ;apexEf$h2[i-1]= pdistantEdgeef$n[i] ;apexEf$d1[i]=( pdistantEdgeef$ws[i]  <<  1 );
 apexEf$d2[i-1]=( pdistantEdgeef$en[i]  >>>  1 );apexEf$ad1[i-1]= pdistantEdgeef$wn[i] ;apexEf$ad2[i-1]=( tmun14  >>>  1 );tmun14= pdistantEdgeef$es[i] ;
  }
  ;
p.prepareBit(apexEf)
 ;
...
// initialisation 
 int tmun09=0,tmun10=0,tmun11=0,tmun12=0;
for (int i = 1; i < pdistantEdgevf$h1.length -1; i++) {
 apexVf$es[i]=( pdistantEdgevf$ad2[i]  <<  1 );apexVf$s[i-1]=( pdistantEdgevf$h1[i]  >>>  1 );apexVf$ws[i-1]=( tmun09  >>>  1 );tmun09= pdistantEdgevf$d1[i] ;
 apexVf$wn[i]= tmun10 ;tmun10= pdistantEdgevf$ad1[i] ;apexVf$n[i]= tmun11 ;tmun11= pdistantEdgevf$h2[i] ;
 apexVf$en[i]=( tmun12  <<  1 );tmun12= pdistantEdgevf$d2[i] ;
  }
  ;
p.prepareBit(apexVf)
 ;
...
// initialisation 
 int tmun23=0,tmun24=0,tmun25=0;
for (int i = 1; i < pngh$e.length -1; i++) {
 commneighbors$e[i]=( pngh$w[i]  <<  1 );commneighbors$se[i-1]= pngh$nw[i] ;commneighbors$sw[i-1]=( pngh$ne[i]  >>>  1 );commneighbors$w[i-1]=( tmun23  >>>  1 );
 tmun23= pngh$e[i] ;commneighbors$nw[i]= tmun24 ;tmun24= pngh$se[i] ;commneighbors$ne[i]=( tmun25  <<  1 );
 tmun25= pngh$sw[i] ;
  }
  ;
...
public static int neighborsEv_1_1GateCount=0;
 public static void neighborsEv_1_1(PrShift p,int [][] pnghhhh,int [][] ver){
 int[] ver$h1=ver[0],ver$h2=ver[1],ver$d1=ver[2],ver$d2=ver[3],ver$ad1=ver[4],ver$ad2=ver[5],pnghhhh$e=pnghhhh[0],pnghhhh$se=pnghhhh[1],pnghhhh$sw=pnghhhh[2],pnghhhh$w=pnghhhh[3],pnghhhh$nw=pnghhhh[4],pnghhhh$ne=pnghhhh[5];


for (int i = 1; i < pnghhhh$e.length -1; i++) {
 ver$h1[i]= pnghhhh$w[i] ;ver$h2[i]=( pnghhhh$e[i]  <<  1 );ver$d1[i]= pnghhhh$nw[i] ;ver$d2[i-1]= pnghhhh$se[i] ;
 ver$ad1[i]= pnghhhh$ne[i] ;ver$ad2[i-1]=( pnghhhh$sw[i]  >>>  1 );
  }
  ;
p.prepareBit(ver)
 ;
