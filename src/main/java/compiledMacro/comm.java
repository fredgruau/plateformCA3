...
// initialisation 
 int tmun07$0=0,tmun07$1=0,tmun07$2=0,tmun07$3=0,tmun08$0=0,tmun08$1=0,tmun08$2=0,tmun08$3=0,tmun09$0=0,tmun09$1=0,tmun09$2=0,tmun09$3=0;
for (int i = 1; i < pngh$e$0.length -1; i++) {
 ...
 ver$w$0[i-1]=( tmun07$0  >>>  1 );ver$w$1[i-1]=( tmun07$1  >>>  1 );ver$w$2[i-1]=( tmun07$2  >>>  1 );ver$w$3[i-1]=( tmun07$3  >>>  1 );
 tmun07$0= pngh$e$0[i] ;tmun07$1= pngh$e$1[i] ;tmun07$2= pngh$e$2[i] ;tmun07$3= pngh$e$3[i] ;
 ver$nw$0[i]= tmun08$0 ;ver$nw$1[i]= tmun08$1 ;ver$nw$2[i]= tmun08$2 ;ver$nw$3[i]= tmun08$3 ;
 tmun08$0= pngh$se$0[i] ;tmun08$1= pngh$se$1[i] ;tmun08$2= pngh$se$2[i] ;tmun08$3= pngh$se$3[i] ;
 ver$ne$0[i]=( tmun09$0  <<  1 );ver$ne$1[i]=( tmun09$1  <<  1 );ver$ne$2[i]=( tmun09$2  <<  1 );ver$ne$3[i]=( tmun09$3  <<  1 );
 tmun09$0= pngh$sw$0[i] ;tmun09$1= pngh$sw$1[i] ;tmun09$2= pngh$sw$2[i] ;tmun09$3= pngh$sw$3[i] ;
  }
  ;
p.prepareBit(ver)
 ;
...
// initialisation 
 int tmun19$0=0,tmun19$1=0,tmun19$2=0,tmun19$3=0,tmun20$0=0,tmun20$1=0,tmun20$2=0,tmun20$3=0;
for (int i = 1; i < pdistantEdgeef$es$0.length -1; i++) {
 apexEf$h1$0[i]=( tmun19$0  <<  1 );apexEf$h1$1[i]=( tmun19$1  <<  1 );apexEf$h1$2[i]=( tmun19$2  <<  1 );apexEf$h1$3[i]=( tmun19$3  <<  1 );
 tmun19$0= pdistantEdgeef$s$0[i] ;tmun19$1= pdistantEdgeef$s$1[i] ;tmun19$2= pdistantEdgeef$s$2[i] ;tmun19$3= pdistantEdgeef$s$3[i] ;
 ...
 apexEf$ad2$0[i-1]=( tmun20$0  >>>  1 );apexEf$ad2$1[i-1]=( tmun20$1  >>>  1 );apexEf$ad2$2[i-1]=( tmun20$2  >>>  1 );apexEf$ad2$3[i-1]=( tmun20$3  >>>  1 );
 tmun20$0= pdistantEdgeef$es$0[i] ;tmun20$1= pdistantEdgeef$es$1[i] ;tmun20$2= pdistantEdgeef$es$2[i] ;tmun20$3= pdistantEdgeef$es$3[i] ;
  }
  ;
p.prepareBit(apexEf)
 ;
...
// initialisation 
 int tmun14=0,tmun15=0;
for (int i = 1; i < pdistantEdgeef$es.length -1; i++) {
 apexEf$h1[i]=( tmun14  <<  1 );tmun14= pdistantEdgeef$s[i] ;apexEf$h2[i-1]= pdistantEdgeef$n[i] ;apexEf$d1[i]=( pdistantEdgeef$ws[i]  <<  1 );
 apexEf$d2[i-1]=( pdistantEdgeef$en[i]  >>>  1 );apexEf$ad1[i-1]= pdistantEdgeef$wn[i] ;apexEf$ad2[i-1]=( tmun15  >>>  1 );tmun15= pdistantEdgeef$es[i] ;
  }
  ;
p.prepareBit(apexEf)
 ;
...
// initialisation 
 int tmun10=0,tmun11=0,tmun12=0,tmun13=0;
for (int i = 1; i < pdistantEdgevf$h1.length -1; i++) {
 apexVf$es[i]=( pdistantEdgevf$ad2[i]  <<  1 );apexVf$s[i-1]=( pdistantEdgevf$h1[i]  >>>  1 );apexVf$ws[i-1]=( tmun10  >>>  1 );tmun10= pdistantEdgevf$d1[i] ;
 apexVf$wn[i]= tmun11 ;tmun11= pdistantEdgevf$ad1[i] ;apexVf$n[i]= tmun12 ;tmun12= pdistantEdgevf$h2[i] ;
 apexVf$en[i]=( tmun13  <<  1 );tmun13= pdistantEdgevf$d2[i] ;
  }
  ;
p.prepareBit(apexVf)
 ;
...
// initialisation 
 int tmun16=0,tmun17=0,tmun18=0;
for (int i = 1; i < pngh$e.length -1; i++) {
 commneighbors$e[i]=( pngh$w[i]  <<  1 );commneighbors$se[i-1]= pngh$nw[i] ;commneighbors$sw[i-1]=( pngh$ne[i]  >>>  1 );commneighbors$w[i-1]=( tmun16  >>>  1 );
 tmun16= pngh$e[i] ;commneighbors$nw[i]= tmun17 ;tmun17= pngh$se[i] ;commneighbors$ne[i]=( tmun18  <<  1 );
 tmun18= pngh$sw[i] ;
  }
  ;
