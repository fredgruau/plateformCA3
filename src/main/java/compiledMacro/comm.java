...
// initialisation 
 int tmun15$0=0,tmun15$1=0,tmun15$2=0,tmun15$3=0,tmun16$0=0,tmun16$1=0,tmun16$2=0,tmun16$3=0,tmun17$0=0,tmun17$1=0,tmun17$2=0,tmun17$3=0;
for (int i = 1; i < pngh$e$0.length -1; i++) {
 ...
 ver$w$0[i-1]=( tmun15$0  >>>  1 );ver$w$1[i-1]=( tmun15$1  >>>  1 );ver$w$2[i-1]=( tmun15$2  >>>  1 );ver$w$3[i-1]=( tmun15$3  >>>  1 );
 tmun15$0= pngh$e$0[i] ;tmun15$1= pngh$e$1[i] ;tmun15$2= pngh$e$2[i] ;tmun15$3= pngh$e$3[i] ;
 ver$nw$0[i]= tmun16$0 ;ver$nw$1[i]= tmun16$1 ;ver$nw$2[i]= tmun16$2 ;ver$nw$3[i]= tmun16$3 ;
 tmun16$0= pngh$se$0[i] ;tmun16$1= pngh$se$1[i] ;tmun16$2= pngh$se$2[i] ;tmun16$3= pngh$se$3[i] ;
 ver$ne$0[i]=( tmun17$0  <<  1 );ver$ne$1[i]=( tmun17$1  <<  1 );ver$ne$2[i]=( tmun17$2  <<  1 );ver$ne$3[i]=( tmun17$3  <<  1 );
 tmun17$0= pngh$sw$0[i] ;tmun17$1= pngh$sw$1[i] ;tmun17$2= pngh$sw$2[i] ;tmun17$3= pngh$sw$3[i] ;
  }
  ;
p.prepareBit(ver)
 ;
...
// initialisation 
 int tmun63$0=0,tmun63$1=0,tmun63$2=0,tmun63$3=0,tmun64$0=0,tmun64$1=0,tmun64$2=0,tmun64$3=0;
for (int i = 1; i < pdistantEdgeef$es$0.length -1; i++) {
 apexEf$h1$0[i]=( tmun63$0  <<  1 );apexEf$h1$1[i]=( tmun63$1  <<  1 );apexEf$h1$2[i]=( tmun63$2  <<  1 );apexEf$h1$3[i]=( tmun63$3  <<  1 );
 tmun63$0= pdistantEdgeef$s$0[i] ;tmun63$1= pdistantEdgeef$s$1[i] ;tmun63$2= pdistantEdgeef$s$2[i] ;tmun63$3= pdistantEdgeef$s$3[i] ;
 ...
 apexEf$ad2$0[i-1]=( tmun64$0  >>>  1 );apexEf$ad2$1[i-1]=( tmun64$1  >>>  1 );apexEf$ad2$2[i-1]=( tmun64$2  >>>  1 );apexEf$ad2$3[i-1]=( tmun64$3  >>>  1 );
 tmun64$0= pdistantEdgeef$es$0[i] ;tmun64$1= pdistantEdgeef$es$1[i] ;tmun64$2= pdistantEdgeef$es$2[i] ;tmun64$3= pdistantEdgeef$es$3[i] ;
  }
  ;
p.prepareBit(apexEf)
 ;
...
// initialisation 
 int tmun51=0,tmun52=0;
for (int i = 1; i < pdistantEdgeef$es.length -1; i++) {
 apexEf$h1[i]=( tmun51  <<  1 );tmun51= pdistantEdgeef$s[i] ;apexEf$h2[i-1]= pdistantEdgeef$n[i] ;apexEf$d1[i]=( pdistantEdgeef$ws[i]  <<  1 );
 apexEf$d2[i-1]=( pdistantEdgeef$en[i]  >>>  1 );apexEf$ad1[i-1]= pdistantEdgeef$wn[i] ;apexEf$ad2[i-1]=( tmun52  >>>  1 );tmun52= pdistantEdgeef$es[i] ;
  }
  ;
p.prepareBit(apexEf)
 ;
...
// initialisation 
 int tmun18=0,tmun19=0,tmun20=0,tmun21=0;
for (int i = 1; i < pdistantEdgevf$h1.length -1; i++) {
 apexVf$es[i]=( pdistantEdgevf$ad2[i]  <<  1 );apexVf$s[i-1]=( pdistantEdgevf$h1[i]  >>>  1 );apexVf$ws[i-1]=( tmun18  >>>  1 );tmun18= pdistantEdgevf$d1[i] ;
 apexVf$wn[i]= tmun19 ;tmun19= pdistantEdgevf$ad1[i] ;apexVf$n[i]= tmun20 ;tmun20= pdistantEdgevf$h2[i] ;
 apexVf$en[i]=( tmun21  <<  1 );tmun21= pdistantEdgevf$d2[i] ;
  }
  ;
p.prepareBit(apexVf)
 ;
...
// initialisation 
 int tmun54=0,tmun55=0,tmun56=0;
for (int i = 1; i < pngh$e.length -1; i++) {
 commneighbors$e[i]=( pngh$w[i]  <<  1 );commneighbors$se[i-1]= pngh$nw[i] ;commneighbors$sw[i-1]=( pngh$ne[i]  >>>  1 );commneighbors$w[i-1]=( tmun54  >>>  1 );
 tmun54= pngh$e[i] ;commneighbors$nw[i]= tmun55 ;tmun55= pngh$se[i] ;commneighbors$ne[i]=( tmun56  <<  1 );
 tmun56= pngh$sw[i] ;
  }
  ;
